...
function calcularExpresso(peso, valorDeclarado) {
  return calcularGenericoComFaixas(tabelaExpresso, peso, valorDeclarado, 0.007, 4.00);
}
...
function calcularGenericoComFaixas(tabela, peso, valorDeclarado, taxaAdValorem, taxaEmissao) {
  const origem = document.getElementById("origem").value;
  const destino = document.getElementById("destino").value;
  const rotasPossiveis = tabela.filter(r =>
  ...
  const capatazia = peso * 0.20;
  const emissao = taxaEmissao;
  const advalorem = valorDeclarado * taxaAdValorem;
